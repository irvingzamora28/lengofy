...
import { WordPuzzleGame, User } from '@/types';
...
interface Props {
    ...
    activeGames: WordPuzzleGame[];
    wsEndpoint: string;
}

export default function WordPuzzleLobby({ auth, activeGames, wsEndpoint }: Props) {
    const [games, setGames] = useState<WordPuzzleGame[]>(activeGames);
    const [selectedLanguagePair, setSelectedLanguagePair] = useState(null);
    const [showDifficultyModal, setShowDifficultyModal] = useState(false);
    const [isSinglePlayer, setIsSinglePlayer] = useState(false);
    const [selectedDifficulty, setSelectedDifficulty] = useState<'easy' | 'medium' | 'hard'>(
        auth.user.word_puzzle_difficulty || 'medium'
    );
    ...
    useEffect(() => {
        ...
        ws.onmessage = (event) => {
            ...
            if (data.type === 'word_puzzle_game_created') {
            ...
            } else if (data.type === 'word_puzzle_game_ended') {
    ...
    const startCreateRoom = () => {
        router.post(
            route('profile.game-settings.update', { redirectRoute: 'games.word-puzzle.lobby' }),
            {
                word_puzzle_difficulty: selectedDifficulty,
            },
            {
                preserveScroll: true,
                onSuccess: () => {
                    router.post(route('games.word-puzzle.create'), {
    ...
    const startSinglePlayerGame = () => {
        router.post(
            ...
            },
            {
                preserveScroll: true,
                onSuccess: () => {
                    router.visit(route('games.word-puzzle.practice'), {
                        method: 'get',
                        data: {
                            difficulty: selectedDifficulty,
                            category: selectedCategory
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">{trans('word_puzzle.lobby')}</h2>}
        >
            <Head title={trans('word_puzzle.lobby')} />
            <div className="py-12">
                                    ...
                                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
                                        <div>
                                            <h2 className="text-xl sm:text-2xl font-bold text-gray-800 dark:text-gray-100 flex items-center">
                                                ...
                                                <span className="hidden sm:inline">{trans('word_puzzle.active_games_plural')}</span>
                                                <span className="sm:hidden">{trans('word_puzzle.active_games')}</span>
                                            </h2>
                                            <p className="text-sm text-gray-600 dark:text-gray-300 mt-1 sm:mt-2">
                                                {trans('word_puzzle.connect_learn_challenge')}
                                            </p>
                                        </div>
                                        <div className="flex space-x-3 w-full sm:w-auto">
                                            ...
                                            >
                                                ...
                                                {trans('word_puzzle.btn_create_new_room')}
                                            </button>
                                            <button
                                            ...
                                            >
                                                ...
                                                {trans('word_puzzle.btn_practice_alone')}
                                            </button>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    {filteredGames.length > 0 ? (
                                                    ...
                                                    <div className="mb-4">
                                                        <div className="flex justify-between items-center mb-4">
                                                            ...
                                                            <span className={`px-2 py-1 rounded text-xs ${
                                                                ...
                                                                {trans(`word_puzzle.modal_difficulty.${game.difficulty}`)}
                                                            </span>
                                                        </div>
                                                        ...
                                                        <Link
                                                            href={route('games.word-puzzle.join', game.id)}
                                                            method="post"
                                                            as="button"
                                                        ...
                                                        >
                                                            <FaPlay className="mr-2 w-4 h-4" /> {trans('word_puzzle.btn_join_game')}
                                                        </Link>
                                                    </div>
                                                </div>
                                            ))}
                                        ...
                                        <div className="text-center py-8">
                                            <p className="text-gray-500 dark:text-gray-400">
                                                {trans('word_puzzle.no_active_games')}
                                            </p>
                                        </div>
                                    )}
                                </div>
            ...
            {showDifficultyModal && (
                <DifficultyModal
                    ...
                    startGame={startGame}
                    easyText={trans('word_puzzle.modal_difficulty.easy_text')}
                    mediumText={trans('word_puzzle.modal_difficulty.medium_text')}
                    hardText={trans('word_puzzle.modal_difficulty.hard_text')}
